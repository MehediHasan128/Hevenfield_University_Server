...
export const generateBatch = async (
  ...
  const isStudentExistOnADepartment = await Student.findOne(
    { academicDepartment: departmentId },
    { _id: 0, batch: 1, id: 1 },
  ).sort({ createdAt: -1 });

  // find last student addmission year, semester code, department code, or batch
  const lastStudentAddmissionYear = isStudentExistOnADepartment?.id.substring(
    0,
    2,
  );
  const lastStudentSemesterCode = isStudentExistOnADepartment?.id.substring(
    2,
    4,
  );
  const lastStudentDepartmentCode = isStudentExistOnADepartment?.id.substring(
    4,
    7,
  );
  ...
  if (
    isStudentExistOnADepartment &&
    lastStudentAddmissionYear === addmissionYear &&
    lastStudentSemesterCode === semesterCode &&
    lastStudentDepartmentCode === departmentCode
  ) {
...
export const generateFacultyId = async (departmentId: Types.ObjectId) => {
  const currentDepartment = await AcademicDepartment.findById(departmentId, {
    _id: 0,
    departmentCode: 1,
  });
...
export const generateAdminId = async (departmentId: Types.ObjectId) => {
